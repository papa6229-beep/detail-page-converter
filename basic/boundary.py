...
규칙은 원본 메인섹션의 생김새를 그대로 따라간다.
    ...
    대표컷 → 요약정보 표 → **3줄요약(큰 색 글씨)** → 패키지 상자 ┃ 본문
...
def find_body_start(entries: list[Entry]) -> tuple[int, str]:
    """(본문이 시작되는 밴드 번호, 사람이 읽을 한 줄).

    못 찾으면 0 을 돌려준다 — 전부를 본문으로 본다. 메인이 비는 것보다
    본문이 길어지는 쪽이 덜 나쁘다. 왜 못 찾았는지는 말한다.
    """
    if not entries:
