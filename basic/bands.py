"""기본형 밴드 — 고도몰 생성기의 픽셀 계층을 그대로 옮긴 것 (legacy 복원본 + 균일행 규칙 1개 추가).
...
def blank_rows(arr: np.ndarray) -> np.ndarray:
    ...
    **가로는 전부 본다.** 예전에는 64열만 띄엄띄엄 찍었다. 그러면 점선 타원이나
    얇은 치수선이 열 사이로 빠져 그 행이 빈 행으로 세어지고, 그림 한가운데가
    잘렸다(벨벳키스의 타원·치수 도해). 폭이 800이든 2000이든 한 번 훑는 값이라
    전부 보는 편이 싸고 정확하다.
    """
    cols = arr
    white = (_lum(cols) >= WHITE_THR).mean(1) >= WHITE_FRAC
    # 기본형 추가 규칙 — **색 상관없이 가로로 균일한 행도 빈 행이다.**
    # 원본에 연보라·하늘색 패널이 흔한데(브루스·글랜스), 흰색만 빈 행으로 보면
    # 패널 전체가 밴드 하나로 뭉친다. 채널별 가로 편차가 작으면 그 행엔 내용이 없다.
    uniform = cols.astype(np.float32).std(axis=1).mean(axis=1) < 4.0
...
def split_bands(arr: np.ndarray, min_gap_px: int = MIN_GAP) -> list[tuple[int, int]]:
    ...
    # 여백 기준은 폭에 비례한다. 800px 원본에서 24px 이던 것이 364px 원본에선 11px 다.
    min_gap = max(4, round(arr.shape[1] * min_gap_px / 800))
