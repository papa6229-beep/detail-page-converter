...
**이 기준선이 뜻하는 것 (헷갈리지 말 것)**

    프롬프트를 **안 바꾼** 회차에서 대표컷이 바뀌면 → 실패. 그 커밋을 되돌린다.
    프롬프트를 **바꾼** 회차는 기준선이 못 지킨다. 모델이 고르는 주체이므로
    프롬프트가 한 글자만 달라져도 답이 흔들린다. 그때는 **사람이 7개를 눈으로
    보고** 괜찮다고 한 뒤에 기준선을 새로 박는다.

즉 **기준선은 코드 회귀를 잡는 도구고, 프롬프트 변경은 눈으로 잡는다.**
모델 호출은 `web._pin` 이 temperature 0 · seed 로 못박아 두었지만, 그것이
보장하는 것은 **같은 프롬프트 안에서의 되풀이**뿐이다.
